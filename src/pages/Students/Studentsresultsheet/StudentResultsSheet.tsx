...
const StudentResultsSheet: React.FC<Props> = ({
  ...
  student = { name: "Kithurshika K", regNo: "EG/2020/2005", gradesByCode: { EE4250: "A+", IS4305: "A" } },
  modulesCountingForGPA = { core: ["EE4250"], electives: ["EE4250", "EE4250"] },
  note = "Note:The results of the module IS4411 was previously released on December 23,2024 and submitted for Senate Approval",
}) => {
  ...
  return (
    <div className="results-page">
        ...
        <div className="rs-top">
          ...
          <div className="legend">
            <div className="legend-row">
              ...
              {gradeRow.map((g) => <span key={g} className="legend-cell">{g}</span>)}
            </div>
            <div className="legend-row">
              <span className="legend-head">Grade Point</span>
              {gpRow.map((g, i) => <span key={i} className="legend-cell">{g}</span>)}
            </div>
          </div>
            ...
            <table className="rs-table">
              ...
              <tbody>
                <tr>
                  ...
                  <td>{student?.regNo}</td>
                  {coreCodes.length
                    ? coreCodes.map((c, i) => <td className="center" key={`cc-${i}`}>{c}</td>)
                    : Array.from({ length: coreCount }).map((_, i) => <td className="center" key={`cc-empty-${i}`}>-</td>)
                  }
                  {elecCodes.length
                    ? elecCodes.map((c, i) => <td className="center" key={`ec-${i}`}>{c}</td>)
                    : Array.from({ length: elecCount }).map((_, i) => <td className="center" key={`ec-empty-${i}`}>-</td>)
                  }
                </tr>
                ...
                <tr>
                  <td colSpan={2}></td>
                  {coreCodes.length
                    ? coreCodes.map((c, i) => <td className="center" key={`cg-${i}`}>{grades[c] ?? "-"}</td>)
                    : Array.from({ length: coreCount }).map((_, i) => <td className="center" key={`cg-empty-${i}`}>-</td>)
                  }
                  {elecCodes.length
                    ? elecCodes.map((c, i) => <td className="center" key={`eg-${i}`}>{grades[c] ?? "-"}</td>)
                    : Array.from({ length: elecCount }).map((_, i) => <td className="center" key={`eg-empty-${i}`}>-</td>)
                  }
                </tr>
              </tbody>
            </table>
            ...
            <section className="rs-sign-exact">
              ...
              <div className="sig-row two-col">
                <div className="cell left">
                  ...
                  <div className="lineBox">
                    <div className="sig-line l-line" />
                  </div>
                  <div className="caption">
                    Senior Assistant Fregistrar,<br />
      ...
      {/* Controls UNDER the sheet (won’t print) */}
