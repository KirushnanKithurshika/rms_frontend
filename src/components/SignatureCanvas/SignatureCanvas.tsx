...
export default function SignatureBoard75x100() {
  ...
  // “Save” => generate PNG and switch to paper-only view (hide canvas & controls)
  const save = () => {
    const c = canvasRef.current!;
    const url = c.toDataURL("image/png");
    setFinalPngUrl(url); // show only the PNG (no download)
  };
  ...
  const handleFile = (file: File) => {
    ...
    reader.onload = () => {
      ...
      image.onload = () => {
        setImg(image);
        // Fit inside current canvas visible size
  ...
  return (
    <div className="sig-wrap">
      ...
      {/* Outside toolbar & uploader */}
      <div className="sig-controls">
        <div className="sig-toolbar-outside">
          <button className="sig-btn" onClick={undo} title="Undo last stroke">Undo</button>
          <button className="sig-btn" onClick={clearAll} title="Clear">Clear</button>
          <button className="sig-btn sig-btn--primary" onClick={save} title="Save as PNG">
            Save
          </button>
        </div>

        <label htmlFor="sig-upload" className="sig-upload-label" title="Upload an image to place on the board">
          Upload Image
        </label>
        <input id="sig-upload" type="file" accept="image/*" onChange={onFileChange} style={{ display: "none" }} />
        <small className="sig-hint">or drag & drop onto the board</small>
      </div>
    </div>
  );
}
